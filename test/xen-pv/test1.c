...
int main(void)
{
	...
	printf("max node:%d possible nodes:%d\n", max, maxmask);
	...
	for (i = max; i >= 0; --i) {
		...
		printf("mbind nodemask:%lu nodesize:%lu\n", *(nodes->maskp), nodes->size);
		...
		printf("get_mempolicy nodemask:%lu nodesize:%lu\n", *(mask->maskp), mask->size);
